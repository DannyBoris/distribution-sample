...
import { getYears, notFalsyAllowZero } from "../utils/helper-functions";
import {
  artistMapper,
...
import AddTrackModalContent from "../components/modal-content/add-tracks";
import DistributionFormSkeleton from "../components/skeletons/distribution-form-skeleton";
...
const DistributionForm = () => {
  ...
  const [commonData, setCommonData] = useState<any>(() => {
    const dataString = localStorage.getItem("common_data");
    return dataString ? JSON.parse(dataString) : null;
  });
  const [artistToUpdate, setArtistToUpdate] = useState<
    (Artist & { index?: number }) | null
  >(null);
  ...
  useEffect(() => {
    setTimeout(() => {
      setRelease(dummy_release(parseInt(params.id)) as Release);
    }, 2000);
  }, []);

  return !release ? (
    <DistributionFormSkeleton />
  ) : (
    <>
            ...
            <Row>
              <Col span={24} style={sharedStyles}>
                <FormSelect
                  ...
                  onChange={(id) => setRelease({ ...release, languageId: id })}
                />
              </Col>
            </Row>
            <Row>
      ...
      <AppModal
        ...
        content={
          <ArtistModalContent
            ...
            languages={commonData.languages}
            onSave={(newArtist: Artist) => {
              if (notFalsyAllowZero(artistToUpdate?.index)) {
                // update contributor
                console.log("contributor");
              } else {
                newArtist["artistName"] = newArtist.name;
                delete newArtist.name;
                setRelease({ ...release, ...newArtist });
              }
              setTimeout(() => {
                // saving artist
                callNotification("Artist saved!");
              }, 1500);
            }}
          />
        }
      ...
      <AppModal
        ...
        content={
          <TrackModalContent
            artists={artists}
            languages={commonData.languages}
            musicStyles={commonData.musicStyles}
      ...
      <AppModal
        formId="add-tracks"
        setOpen={setAddTrackModalOpen}
        title="Track"
        open={addTrackModalOpen}
        content={
          <AddTrackModalContent
            onSave={(tracks) => {
              console.log(tracks);
            }}
          />
